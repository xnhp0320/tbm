...
/*
 * C binding for the IPv4 / stride-5 Tree Bitmap.
 *
 * Layout of FibTbm / FibCidr is generated by the C3 compiler and may change
 * between compiler or library versions. Treat it as an opaque blob: allocate
 * a zeroed FibTbm, call tbm_init, and only touch it through these functions.
 *
 * Faults: tbm_insert / tbm_remove / tbm_lookup / tbm_init return NULL on
 * success and a c3fault_t on failure. Use tbm_fault_name() for a short name.
 */
...
/* FUNCTIONS */
/* Requires a zeroed FibTbm. Re-init only after tbm_free. */
extern c3fault_t tbm_init(FibTbm* self, size_t nodes_cnt);
...
/* Temporary allocator string; valid until the next temp-allocator reset. */
extern const char* tbm_fault_name(c3fault_t f);
